...
pub async fn cmd_global(cli: &Cli) {
    ...
    println!("{}", gray("Global Chat — over workspace descriptions"));
    println!("{}", gray("Commands: /quit /model <Flash|Pro> /thinking /save /load /history /clear /help"));
    println!();
    ...
    loop {
        ...
        let ctx = chat::build_global_chat_context(&ws.papers);
